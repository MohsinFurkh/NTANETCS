...
export default function ByDifficultyPracticePage() {
  ...
  // Fetch available difficulties for selection
  useEffect(() => {
    async function fetchDifficulties() {
      try {
        const res = await fetch('/api/practice/by-difficulty');
        const data = await res.json();
        const difficulties = data.map((item: { difficulty: string }) => item.difficulty).filter(Boolean);
        setAvailableDifficulties(difficulties);
        if (difficulties.length > 0 && difficulty === null) setDifficulty(difficulties[0]);
      } catch (error) {
  ...
  useEffect(() => {
    async function fetchQuestions() {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/practice/questions?difficulty=${encodeURIComponent(difficulty)}`);
        const data = await res.json();
        setQuestions(data);
    ...
    if (session && difficulty) fetchQuestions();
  }, [session, difficulty]);
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">No Questions Available for Difficulty {difficulty}</h1>
          <button
            onClick={() => router.back()}
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold">Practice by Difficulty</h1>
