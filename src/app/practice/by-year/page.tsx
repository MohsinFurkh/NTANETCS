...
export default function ByYearPracticePage() {
  ...
  useEffect(() => {
    async function fetchYears() {
      try {
        ...
        const years = data.map((item: { year: number }) => item.year).filter(Boolean);
  ...
  useEffect(() => {
    async function fetchQuestions() {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/practice/questions?year=${year}`);
        const data = await res.json();
        setQuestions(data);
    ...
    if (session && year) fetchQuestions();
  }, [session, year]);
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">No Questions Available for Year {year}</h1>
          <button
            onClick={() => router.back()}
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold">Practice by Year</h1>
